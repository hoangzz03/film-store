...
import { getAllOrder } from './utils/order.utils';
...
const Products: React.FC = () => {
  ...
  const [filterBy, setFilterBy] = useState<'all' | 'orderId' | 'name' | 'role' | 'status'>('all');
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      console.error("Lỗi khi lấy dữ liệu don hang:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredProduct = useMemo(() => {
    ...
    return data.filter(order => {
      if (filterBy === 'name') return order.orderId.toLowerCase().includes(term);
        ...
        <Table>
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">CREATEAT</th>
          ...
          <tbody>
            {loading ? (
              ...
              filteredProduct.map((order) => (
                <ItemOrder data={order} key={order.id} handleDelete={handleDelete} />
              ))
            ) : (
              <tr>
